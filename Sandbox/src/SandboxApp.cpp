...
public:
    ...
    void OnEvent(April::Event& event) override
    {
        April::EventDispatcher dispatcher(event);

        dispatcher.Dispatch<April::KeyPressedEvent>(AL_BIND_EVENT_FN(ExampleLayer::OnKeyPressedEvent));
    }

    bool OnKeyPressedEvent(April::KeyPressedEvent& event)
    {
        if (event.GetKeyCode() == AL_KEY_LEFT)
        {
            m_CameraPosition.x += m_CameraSpeed;
        }
        if (event.GetKeyCode() == AL_KEY_RIGHT)
        {
            m_CameraPosition.x -= m_CameraSpeed;
        }
        if (event.GetKeyCode() == AL_KEY_DOWN)
        {
            m_CameraPosition.y += m_CameraSpeed;
        }
        if (event.GetKeyCode() == AL_KEY_UP)
        {
            m_CameraPosition.y -= m_CameraSpeed;
        }
        return false;
